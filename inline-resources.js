  ...
  return content.replace(/styleUrls:\s*(\[[\s\S]*?\])/gm, function (m, styleUrls) {
    ...
    return 'styles: ['
      + urls.map(styleUrl => {
        ...
        // compyle style content with node-sass before inlining
        const sassCompiled = styleContent && styleContent.length > 0 ? sass.renderSync({
          data: styleContent,
          outputStyle: 'compressed',
          includePaths: ['./src']
        }).css.toString() : '';
        // ++
        const shortenedStyle = sassCompiled
          .replace(/([\n\r]\s*)+/gm, ' ')
          .replace(/"/g, '\\"');
